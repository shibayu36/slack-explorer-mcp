...
- メッセージ検索 (`search_messages`)
  ...
  - パラメータ
    - `query`: 基本検索クエリ。無印の修飾子は専用フィールドへ（`query` への直書きはエラー）。除外指定（`-in:#channel` のような `-` プレフィックス付き）はこのフィールドに書けます
...
- ファイル検索 (`search_files`)
  ...
  - パラメータ
    - `query`: 基本検索クエリ。無印の修飾子は専用フィールドへ（`query` への直書きはエラー）。除外指定（`-type:pdfs` のような `-` プレフィックス付き）はこのフィールドに書けます
